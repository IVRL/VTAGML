...
class NYUv2Dataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        task = index // self.num_img
        rgb_image = index % self.num_img
        ...
        if self.rgb_transform is not None:
            random.seed(seed)
            img = torch.load(os.path.join(self.folder("rgb"), self._files[rgb_image])) # self.rgb_images[rgb_image, :,:,:]#
        ...
        label = self._get_task_label(task, rgb_image, state)

        return rgb, label, task
    ...
    def __len__(self):
        return len(self._files) * self.num_tasks
