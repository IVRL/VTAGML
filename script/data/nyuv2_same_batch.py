...
class NYUv2SameBatchDataset(Dataset):
    """
    ...
    transformation ends in a tensor. Task name: "segmentation"

    3. Surface Normals: 3 channels, with values in [0, 1]. Task name: "surface_normals"

    4. Depth Images: 1 channel with floats representing the distance in meters.
    Conversion will happen automatically if transformation ends in a tensor. Task name: "depth"
    """
    ...
    def __init__(
        ...
        self.rgb_transform2 = rgb_transform2
        ...
        self._files = sorted(os.listdir(os.path.join(root, f"{self._split}_rgb_pt")))
        self.num_img = len(self._files)
        
        self.num_tasks = len(tasks)
        self.tasks = tasks
        
        self.task_dict = self._get_task_dict()
        
        self.folder = lambda name: os.path.join(self.root, f"{self._split}_{name}_pt")
        
        self.seg_images = torch.load(f"{root}/combined/{self._split}_seg13_pt.pt")
        self.depth_images = torch.load(f"{root}/combined/{self._split}_depth_pt.pt")
    ...
    def __getitem__(self, index: int):
        ...
        if self.rgb_transform is not None:
            random.seed(seed)
            img = torch.load(os.path.join(self.folder("rgb"), self._files[rgb_image]))
            ### https://github.com/pytorch/vision/issues/9#issuecomment-789308878
            state = torch.get_rng_state()
            rgb = self.rgb_transform(img)
            if self.rgb_transform2 is not None:
                rgb = self.rgb_transform2(rgb)

        label_seg = self._get_task_label(0, rgb_image, state)
        label_depth = self._get_task_label(1, rgb_image, state)

        return torch.stack([rgb,rgb]), torch.stack([label_seg, label_depth]), torch.LongTensor([0,1])
    ...
    def _get_task_label(self, task, rgb_image, state):
        seed = random.randrange(sys.maxsize)
        
        task_type = self.task_dict[task]
        if task_type == SEG:
            if self.seg_transform is not None:
                random.seed(seed)
                img = self.seg_images[rgb_image, :,:,:]
        ...
        if task_type == SN: # kontrol et
            if self.sn_transform is not None:
                random.seed(seed)
                img = self.rgb_images[rgb_image, :,:,:]
                torch.set_rng_state(state)
        ...
        if task_type == DEP:
            if self.depth_transform is not None:
                random.seed(seed)
                img = self.depth_images[rgb_image, :,:,:]
                torch.set_rng_state(state)
            ...
            for split in ["train", "test"]:
                for part, transform in zip(
                    ...
                    if transform is None:
                        continue
                    path = os.path.join(self.root, f"{split}_{part}_pt")
