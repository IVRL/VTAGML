...
def train(model, train_loader, test_loader, optimizer, scheduler, criterion, epochs, tensorboard_name, seg_weight, depth_weight, start_epoch = 0, device = "cuda", tb_writer=None):
            ...
            for i, (img, label, task_id) in enumerate(test_loader, 0):
                ...
                for j, unique_task_id in enumerate(unique_task_ids_list):
                    ...
                    if unique_task_id == 0:
                        ...
                        a = criterion[unique_task_id](logit_task.view(B,14,-1), label_task.long().view(B,-1))
                        ...
                        a = criterion[unique_task_id](logit_task, label_task)#* len(logit_task)
...
def get_dataloaders(tasks, batch_size):
    ...
    train_dataset = NYUv2SameBatchDataset(root="./data/nyuv2", tasks=tasks, download=False, train=True,
...
def main():
    ...
    train_dataloader, test_dataloader = get_dataloaders(tasks, batch_size)
    ...
    model = SwinTransformer(img_size=224,
                                ...
                                use_conditional_layer = config["use_conditional_layer_norm"])
    ...
    train(model, train_dataloader, test_dataloader, optimizer, scheduler, criterion, epochs, config["experiment_name"], config["seg_weight"], config["depth_weight"], start_epoch, device, tb_writer)
